...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
{
    ...
    if (!result)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 lfg entrance positions. DB table `lfg_dungeon_template` is empty!");
        return;
    }
    ...
    for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
    {
        ...
        if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f && dungeon.y == 0.0f && dungeon.z == 0.0f)
        {
            ...
            if (!at)
            {
                sLog->outError(LOG_FILTER_SQL, "Failed to load dungeon %s (Id: %u), cant find areatrigger for map %u", dungeon.name.c_str(), dungeon.id, dungeon.map);
                continue;
            }
...
void LFGMgr::Update(uint32 diff)
{
    ...
    for (LfgQueueContainer::iterator it = QueuesStore.begin(); it != QueuesStore.end(); ++it)
        if (uint8 newProposals = it->second.FindGroups())
            sLog->outDebug(LOG_FILTER_LFG, "Found %u new groups in queue %u", newProposals, it->first);
...
void LFGMgr::InitializeLockedDungeons(Player* player, uint8 level /* = 0 */)
{
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
    {
        ...
        */
        ...
        if (lockData)
            lock[dungeon->Entry()] = LfgLockInfoData(lockData, dungeon->requiredItemLevel, player->GetAverageItemLevel());

			
		if (!lockData && reqItemLevel && reqItemLevel > PlrIlvl)
            lockData = LFG_LOCKSTATUS_TOO_LOW_GEAR_SCORE;
    }
...
void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons, const std::string& comment)
{
    ...
    if (joinData.result != LFG_JOIN_OK)
    {
        sLog->outDebug(LOG_FILTER_LFG, "%u joining with %u members. Result: %u, Dungeons: %s",
    ...
    if (isRaid)
    {
        sLog->outDebug(LOG_FILTER_LFG, "%u trying to join raid browser and it's disabled.", GUID_LOPART(guid));
        return;
    }
    ...
    sLog->outDebug(LOG_FILTER_LFG, "%u joined (%s), Members: %s. Dungeons (%u): %s", GUID_LOPART(guid),
...
void LFGMgr::LeaveLfg(uint64 guid)
{
    ...
    sLog->outDebug(LOG_FILTER_LFG, "%u left (%s)", GUID_LOPART(guid), guid == gguid ? "group" : "player");
...
void LFGMgr::UpdateProposal(uint32 proposalId, uint64 guid, bool accept)
{
    ...
    sLog->outDebug(LOG_FILTER_LFG, "Player %u, Proposal %u, Selection: %u", GUID_LOPART(guid), proposalId, accept);
    if (!accept)
    {
...
void LFGMgr::RemoveProposal(LfgProposalContainer::iterator itProposal, LfgUpdateType type)
{
    ...
    sLog->outDebug(LOG_FILTER_LFG, "Proposal %u, state FAILED, UpdateType %u", itProposal->first, type);
    ...
    for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin(); it != proposal.players.end(); ++it)
    {
        ...
        if (toRemove.find(gguid) != toRemove.end())         // Didn't accept or in same group that someone that didn't accept
        {
            ...
            if (it->second.accept == LFG_ANSWER_DENY)
            {
                ...
                sLog->outDebug(LOG_FILTER_LFG, "%u didn't accept. Removing from queue and compatible cache", GUID_LOPART(guid));
            }
            else
            {
                updateData.updateType = LFG_UPDATETYPE_REMOVED_FROM_QUEUE;
                sLog->outDebug(LOG_FILTER_LFG, "%u in same group that someone that didn't accept. Removing from queue and compatible cache",  GUID_LOPART(guid));
            }
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Readding %u to queue.",  GUID_LOPART(guid));
...
void LFGMgr::TeleportPlayer(Player* player, bool out, bool fromOpcode /*= false*/)
{
    ...
    if (!dungeon)
    {
        sLog->outDebug(LOG_FILTER_LFG, "Player %s not in group/lfggroup or dungeon not found!",
    ...
    if (out)
    {
        sLog->outDebug(LOG_FILTER_LFG, "Player %s is being teleported out. Current Map %u - Expected Map %u",
    ...
    sLog->outDebug(LOG_FILTER_LFG, "Player %s is being teleported in to map %u "
...
void LFGMgr::FinishDungeon(uint64 gguid, const uint32 dungeonId)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_LFG, "Group %u finished dungeon %u but queued for %u", GUID_LOPART(gguid), dungeonId, gDungeonId);
        return;
    }

    if (GetState(gguid) == LFG_STATE_FINISHED_DUNGEON) // Shouldn't happen. Do not reward multiple times
    {
        sLog->outDebug(LOG_FILTER_LFG, "Group: %u already rewarded", GUID_LOPART(gguid));
        return;
    }
    ...
    for (LfgGuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Group: %u, Player: %u already rewarded", GUID_LOPART(gguid), GUID_LOPART(guid));
            continue;
        }
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Group: %u, Player: %u dungeon %u is not random or seasonal", GUID_LOPART(gguid), GUID_LOPART(guid), rDungeonId);
            continue;
        }
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Group: %u, Player: %u not found in world", GUID_LOPART(gguid), GUID_LOPART(guid));
            continue;
        }
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Group: %u, Player: %u is in map %u and should be in %u to get reward", GUID_LOPART(gguid), GUID_LOPART(guid), player->GetMapId(), mapId);
            continue;
        }
        ...
        // Give rewards
        sLog->outDebug(LOG_FILTER_LFG, "Group: %u, Player: %u done dungeon %u, %s previously done.", GUID_LOPART(gguid), GUID_LOPART(guid), GetDungeon(gguid), done? " " : " not");
...
LfgState LFGMgr::GetState(uint64 guid)
{
    ...
    if (IS_GROUP_GUID(guid))
    {
        ...
        sLog->outTrace(LOG_FILTER_LFG, "Group: %u, State: %u", GUID_LOPART(guid), state);
    }
    else
    {
        state = PlayersStore[guid].GetState();
        sLog->outTrace(LOG_FILTER_LFG, "Player: %u, State: %u", GUID_LOPART(guid), state);
    }
...
uint32 LFGMgr::GetDungeon(uint64 guid, bool asId /*= true */)
{
    ...
    sLog->outTrace(LOG_FILTER_LFG, "Group: %u, asId: %u, Dungeon: %u", GUID_LOPART(guid), asId, dungeon);
    return dungeon;
}
...
uint32 LFGMgr::GetDungeonMapId(uint64 guid)
{
    ...
    sLog->outTrace(LOG_FILTER_LFG, "Group: %u, MapId: %u (DungeonId: %u)", GUID_LOPART(guid), mapId, dungeonId);
...
uint8 LFGMgr::GetRoles(uint64 guid)
{
    ...
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, Role: %u", GUID_LOPART(guid), roles);
    return roles;
}

const std::string& LFGMgr::GetComment(uint64 guid)
{
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, Comment: %s", GUID_LOPART(guid), PlayersStore[guid].GetComment().c_str());
    return PlayersStore[guid].GetComment();
}

LfgDungeonSet const& LFGMgr::GetSelectedDungeons(uint64 guid)
{
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, Selected Dungeons: %s", GUID_LOPART(guid), ConcatenateDungeons(PlayersStore[guid].GetSelectedDungeons()).c_str());
    return PlayersStore[guid].GetSelectedDungeons();
}

LfgLockMap const& LFGMgr::GetLockedDungeons(uint64 guid)
{
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, LockedDungeons.", GUID_LOPART(guid));
...
uint8 LFGMgr::GetKicksLeft(uint64 guid)
{
    ...
    sLog->outTrace(LOG_FILTER_LFG, "Group: %u, Kicks left: %u", GUID_LOPART(guid), kicks);
    return kicks;
}
...
void LFGMgr::RestoreState(uint64 guid, char const* debugMsg)
{
    if (IS_GROUP_GUID(guid))
    {
        ...
        sLog->outTrace(LOG_FILTER_LFG, "Group: %u (%s), State: %s, Old state: %s",
    ...
    else
    {
        ...
        sLog->outTrace(LOG_FILTER_LFG, "Player: %u (%s), State: %s, Old state: %s",
            GUID_LOPART(guid), debugMsg, GetStateString(data.GetState()).c_str(),
            GetStateString(data.GetOldState()).c_str());
...
void LFGMgr::SetState(uint64 guid, LfgState state)
{
    ...
    {
        LfgGroupData& data = GroupsStore[guid];
        sLog->outTrace(LOG_FILTER_LFG, "Group: %u, New state: %s, Previous: %s, Old state: %s",
    ...
    {
        LfgPlayerData& data = PlayersStore[guid];
        sLog->outTrace(LOG_FILTER_LFG, "Player: %u, New state: %s, Previous: %s, OldState: %s",
            GUID_LOPART(guid), GetStateString(state).c_str(), GetStateString(data.GetState()).c_str(),
            GetStateString(data.GetOldState()).c_str());
...
{
    sLog->outTrace(LOG_FILTER_LFG, "Group: %u, Dungeon: %u", GUID_LOPART(guid), dungeon);
    GroupsStore[guid].SetDungeon(dungeon);
}

void LFGMgr::SetRoles(uint64 guid, uint8 roles)
{
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, Roles: %u", GUID_LOPART(guid), roles);
    PlayersStore[guid].SetRoles(roles);
}

void LFGMgr::SetComment(uint64 guid, std::string const& comment)
{
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, Comment: %s", GUID_LOPART(guid), comment.c_str());
    PlayersStore[guid].SetComment(comment);
}

void LFGMgr::SetSelectedDungeons(uint64 guid, LfgDungeonSet const& dungeons)
{
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, Dungeons: %s", GUID_LOPART(guid), ConcatenateDungeons(dungeons).c_str());
...
void LFGMgr::DecreaseKicksLeft(uint64 guid)
{
    ...
    sLog->outTrace(LOG_FILTER_LFG, "Group: %u, Kicks: %u", GUID_LOPART(guid), GroupsStore[guid].GetKicksLeft());
}

void LFGMgr::RemovePlayerData(uint64 guid)
{
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u", GUID_LOPART(guid));
...
{
    sLog->outTrace(LOG_FILTER_LFG, "Group: %u", GUID_LOPART(guid));
...
uint8 LFGMgr::GetTeam(uint64 guid)
{
    ...
    sLog->outTrace(LOG_FILTER_LFG, "Player: %u, Team: %u", GUID_LOPART(guid), team);
    return team;
}
