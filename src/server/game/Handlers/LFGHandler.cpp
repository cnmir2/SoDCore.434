...
void BuildPlayerLockDungeonBlock(WorldPacket& data, lfg::LfgLockMap const& lock)
{
    ...
    for (lfg::LfgLockMap::const_iterator it = lock.begin(); it != lock.end(); ++it)
    {
        ...
        data << uint32(it->second.lockData);          // Lock status
...
void WorldSession::SendLfgJoinResult(lfg::LfgJoinResultData const& joinData)
{
    ...
    for (lfg::LfgLockPartyMap::const_iterator it = joinData.lockmap.begin(); it != joinData.lockmap.end(); ++it)
    {
        ...
        {
		 sLog->outTrace(LOG_FILTER_LFG, "SendLfgJoinResult:: PlayerGUID: " UI64FMTD " DungeonID: %u Lock status: %u Required itemLevel: %u Current itemLevel: %f",
			uint64(playerGuid), (itr->first & 0x00FFFFFF), itr->second.lockData, itr->second.requiredItemLevel, itr->second.currentItemLevel);
			
            data << uint32(itr->second.lockData);             // Lock status
