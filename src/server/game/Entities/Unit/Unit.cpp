    ...
    if (damageInfo->damage)
    {
        ...
        for (AuraEffectList::const_iterator dmgShieldItr = vDamageShieldsCopy.begin(); dmgShieldItr != vDamageShieldsCopy.end(); ++dmgShieldItr)
        {
            ...
            if (Unit* caster = (*dmgShieldItr)->GetCaster())
            {
                ...
                damage = SpellDamageBonusTaken(caster, i_spellProto, damage, SPELL_DIRECT_DAMAGE, 1, caster->GetGUID());
            }
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, uint64 casterGUID)
{
 if (!spellProto || damagetype == DIRECT_DAMAGE)
return pdamage;
int32 TakenTotal = 0;
float TakenTotalMod = 2.5f;
float TakenTotalCasterMod = 1.0f;
