    ...
    public:
        ...
        struct npc_left_legAI: public ScriptedAI
        {
                ...
                void DamageTaken(Unit* who, uint32& damage)
                {
                    ...
                    if (side >= me->GetMaxHealth() * 0.01)
                        if (GetRhyo())
                        {
                            SetAltPowerForPlayers(float(-1 * (side / (me->GetMaxHealth() * 0.01))));
                            GetRhyo()->StopMoving();
                            GetRhyo()->SetFacingTo(
                                    GetRhyo()->GetOrientation() - ((side / (me->GetMaxHealth() * 0.01)) * 0.01f));
                            side /= me->GetMaxHealth() * 0.01;
                        }

                    if (damage && GetRhyo() && GetRight())
                    {
                        int32 health = me->GetHealth() + GetRight()->GetHealth();
